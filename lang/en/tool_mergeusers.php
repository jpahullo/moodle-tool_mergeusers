...
$string['enableadhocmerge'] = 'Enable adhoc task for web merges';
$string['enableadhocmerge_desc'] = 'This only affects merges started from the web interface. CLI/CRON-driven merges (e.g. via cli/climerger.php or a custom gathering) are never affected by this setting and always run synchronously. Web service merges (tool_mergeusers_enqueue_merge_request) are never affected by this setting either and always run asynchronously. When enabled, a web merge is queued as a background adhoc task and processed the next time Moodle cron runs, instead of running immediately within the web request - avoiding page timeouts on large merges.';
...
$string['origin:ws'] = 'Web service';
$string['originonlog'] = 'Origin';
...
$string['renamelogaction'] = 'Updated {$a->field} from "{$a->oldvalue}" to "{$a->value}" instead of merging, because the user to keep did not exist yet.';
...
$string['renamewhenmissingtarget_desc_username'] = 'When a merge is requested by a login identifier and the user to keep does not exist yet but the user to remove does, the user to remove\'s username is renamed to the value searched for the user to keep, instead of failing. Currently applies when searching by username. Only takes effect for a request queued via the web service (tool_mergeusers_enqueue_merge_request) once its adhoc task actually executes - never for a web-triggered merge (whether immediate, or itself queued via "Enable adhoc task for web merges") nor for a CLI/gathering merge, none of which can end up renaming instead of merging.';
...
$string['wstousernotfoundyet'] = 'This user does not exist yet. If it still does not exist when this request is processed, and renaming is allowed at that point, the user to remove will be renamed instead of doing a full merge.';
